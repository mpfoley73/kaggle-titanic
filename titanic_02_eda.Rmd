...
* `Sex` is a moderating variable for `SibSp`, `ParCh`, and `NetSurv`.
...
Define handy plot functions.

```{r}
plot_bin <- function(dat, rows, predictor, breaks) {
  dat[rows, ] %>% 
    mutate(bin = cut(!!sym(predictor), breaks = breaks, dig.lab = 2)) %>%
    count(Sex, bin, Survived) %>%
    ungroup() %>% 
    group_by(Sex, bin) %>% 
    mutate(
      N = sum(n), 
    ...
    filter(Survived == "Yes") %>%
  ggplot(aes(x = bin, y = log_odds, fill = N)) +
    geom_col() +
    theme(axis.text.x = element_text(angle = 90)) +
    labs(title = paste0("Survival vs ", predictor),
         x = predictor,
         caption = paste0(predictor, " breaks = ", breaks, ".")) +
    facet_wrap(~Sex)
}
```

```{r}
plot_prob2 <- function(dat, fmla, predictor, color){
  ...
  #   ggplot(aes(x = !!sym(predictor), color = !!sym(color))) +
  ...
  #   labs(title = fmla, 
  #        y = "Probability", 
  #        subtitle = paste0("Accuracy = ", scales::percent(fit$results$Accuracy))) +
  #   facet_wrap(~ Sex)
}
```
...
```{r}
plot_bin(full, train_index, "Age", 20)
```

It looks like for both males and females, survival odds fall until around age 16, increase until age 32, fall through age 45, then increase again.  What if we create an `AgeCohort` with theses ranges and make it an interaction with `Age`?
...
dat %>% plot_prob2("Survived ~ Sex * Age*AgeCohort", "Age", "AgeCohort")
mdl <- train(
  Survived ~ Sex * AgeCohort*Age,
  dat,
...
summary(mdl)
mdl$finalModel$fitted.values
dat$p <- mdl$finalModel$fitted.values
dat$bin <- cut(dat$Age, breaks = 20)
dat %>% 
    group_by(Sex, bin, Survived) %>%
...
I might have been wrong about the increasing survival odds after age 45.  For men, almost older person died.  For women, almost everyone lived. Maybe it needs to be a 3-way interaction?

```{r}
dat %>% plot_prob2("Survived ~ Sex*AgeCohort*Age", "Age", "AgeCohort")
```

## SibSp

```{r}
plot_bin(full, train_index, "SibSp", 1)
```

Males traveling alone have particularly low survival odds.  Their odds are better if in a pair. For females, it's the opposite.  The n-size for `SibSp` >= 3 seems too low to infer much.  What if we create an `SibSp0` flag?

```{r}
dat <- full[train_index, ] %>%
  mutate(
    SibSp0 = if_else(SibSp == 0, "yes", "no")
  )
dat %>% plot_prob2("Survived ~ Sex*SibSp0*SibSp", "SibSp", "SibSp0")
```
...
Accuracy is 0.790.  All coefficient estimates are significant.  Now what happens if I collapse `FarePerPass`?
...
The accuracy stayed the same at 0.790.  The coefficient estimates are still significant (even more so for `Sex`).  It probably doesn't make sense to collapse `FarePerPass`.
...
No difference in accuracy.  The p-value for the interaction term is significant, so I should probably add it to the model.
