...
subtitle: "Step 3: Regularization"
...
The ridge model run with the full set of predictors had a holdout set accuracy of 0.8192, sensitivity of 0.6029, specificity of 0.9541, and AUC of 0.8821.
...
mdl_ridge$bestTune
```
`Parch`, `FarePerPass`, `Embarked`, and `Employee` fail the significance test.  `varImp()` ranks the coefficients by the absolute value of the coefficients.  `NetSurv` is most important.
...
rcpe_final <- recipe(Survived ~ ., data = training_80[, mdl_vars_final]) %>%
  update_role(PassengerId, new_role = "id variable") %>%
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_center(Age, SibSp, TicketN, NetSurv) %>%
  step_scale(Age, SibSp, TicketN, NetSurv) %>%
  step_interact(terms = ~
                  starts_with("Sex_"):starts_with("Pclass_") +
                  starts_with("Sex_"):starts_with("Embarked_") +
                  TicketN:starts_with("TicketNCohort_") +
