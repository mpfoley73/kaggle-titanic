...
In this section I will look at data distributions (univariate analysis). For factor variables, I am interested in which have near-zero-variance. For quantitative variables, I am looking for significant skew. Classify the predictors by data type.

```{r}
preds <- full %>% 
  select(-Survived, -PassengerId, -Surname, -Name, -Ticket, -Cabin) %>% 
  colnames()
...
Inspect each factor variable, looking for near-zero-variance predictors. I won’t remove them here, but I might remove them in the modeling pre-process step if they cause zero-variance in cv folds. `nearZeroVar()` defines near-zero variance as a frequency ratio (ratio of the most common value frequency to the second most common value frequency) >= 19 and a percent of unique values <= 10%. (DataCamp course Machine Learning Toolbox suggests more aggressive thresholds of frequency ratio >= 2 and percent of unique values <= 20%).
...
Ideally, you want the vars to land in the top left quadrant. The problematic predictors are `Title = "Master"`, `Employee`, and `Deck = "A"`.  
...
for(i in 1:4) {
  print(plot_ggpairs(preds[(1:3) + 3*i]))
}
```
