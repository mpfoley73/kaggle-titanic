...
* Create grouping variable `AgeCohort`. It should interact with `Age` and `Sex`.
* `Title` is redundant to `AgeCohort` and `Sex` and I should not include it in the models.
* `Pclass` should interact with `Sex`.
* Create grouping variable `TicketNCohort`.  It should interact with `TicketN`.
* `Embarked` should interact with `Sex`.
* Create net survivors predictor `NetSurv` with values truncated at -1 and +1.
...
```{r}
save(full, train_index, preds, mdl_vars, file = "./titanic_02.RData")
```
