...
```{r}
plot_bin <- function(dat, rows, predictor, bin_size) {
  dat[rows, ] %>% 
    mutate(bin = floor(!!sym(predictor) / bin_size) * bin_size) %>%
  ...
  ggplot(aes(x = bin, y = log_odds, fill = N)) +
    ...
    labs(title = paste0("Survival vs ", predictor),
         caption = paste0(predictor, " bin size = ", bin_size, ".")) +
    facet_wrap(~Sex)
}
```
...
```{r}
plot_bin(full, train_index, "Age", 4)
```
