...
`Fare` is the total price of the ticket, but there can be multiple passengers per ticket.  `Fare` is thus a multiple of the number of passengers on the ticket.  Let's untangle that by 1) creating a variable for number of passengers on the ticket, and 2) calculating the price per ticket.
...
`PassengerId` 1044 was a third-class traveler from Southampton traveling alone.  How much did passengers in this profile typically pay?

```{r}
full %>% filter(Embarked == "S" & Pclass == 3) %>% 
  mutate(FarePerPass = Fare / TicketN) %>% select(FarePerPass) %>% summary()
```
...
full %>% ggplot(aes(x = FarePerPass, color = Pclass)) + geom_density() +
...
After a little research [here](https://www.encyclopedia-titanica.org/titanic-passenger-list/), it appears that [nine of the passengers](https://titanic.fandom.com/wiki/Guarantee_Group) who paid $0 were from the Guarantee Group from Harland & Wolff, the shipbuilding firm.  They were regarded as crew.  Of the others, [Reuchlin](https://www.encyclopedia-titanica.org/titanic-victim/johan-george-reuchlin.html) (PassengerId 823) was sort of an employee. So were [Fry, Harrison, and Ismay](https://www.encyclopedia-titanica.org/titanic-survivor/j-bruce-ismay.html) (816, 264, and 1264).  Perhaps the best way to deal with these passengers is to create a new employee variable, `Emp`, set their fare to `NA`, and impute a "market-rate" fare.

```{r}
full <- full %>% 
  mutate(
    Emp = if_else(Fare == 0, 1, 0),
    Fare = if_else(Fare == 0, as.numeric(NA), Fare)
  )
```

I'll use Mice to impute.
...
```{r}
data.frame(full = full$Fare, mice = mice_cmpl$Fare) %>%
...
How do survival chances relate to `FarePerPass`?

```{r}
full[train_index, ] %>%
  mutate(bin = cut(FarePerPass, breaks = 10)) %>%
...
Deck B!  I feel better about that assignment.  One last look at the decks.  Here are the survival rates.
...
plot_surv_grp <- function(dat, x_grp, facet_grp){
  dat %>%
    count(!!sym(facet_grp), !!sym(x_grp), Survived) %>%
    ungroup() %>% 
    group_by(!!sym(facet_grp), !!sym(x_grp)) %>%
    mutate(Pct = if_else(Survived == 1, n / sum(n), as.numeric(NA))) %>%
    ggplot(aes(x = !!sym(x_grp), y = n, 
    ...
    theme(legend.position = "top") +
    labs(
      title = paste0("Survival Probability by ", facet_grp, " and ", x_grp),
      fill = "Survived"
    ) +
    facet_wrap(vars(!!sym(facet_grp)), nrow = 3)
}
full[train_index, ] %>%
