...
Here is the survival log-odds for `Age`.  Children of both sexes had relatively good survival odds.  For males, the children distort a generally positive relationship with age.  The female relationship seems okay.
...
plot_bin(dat, x_var = "Age", facet_var = "Sex", fmla = fmla)
```

So maybe there should be an `AgeCohort` factor variable that moderates `Age`.
...
dat$pred <- mdl$finalModel$fitted.values  
plot_bin(dat, x_var = "Age", facet_var = "Sex", fmla = fmla)
dat %>% filter(Sex == "male") %>% count(Age, Survived) %>% pivot_wider(names_from = "Survived", values_from = "n")
```
